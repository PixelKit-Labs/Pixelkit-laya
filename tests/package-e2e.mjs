...
test("the packed module installs and works in an external ESM TypeScript project", async (t) => {
  const workDir = await mkdtemp(join(tmpdir(), "pixelkit-laya-package-"));
  ...
  for (const required of ["README.md", "LICENSE", "NOTICE", "package.json", "dist/index.js", "dist/index.d.ts", "dist/mobile.js", "dist/mobile.d.ts"]) {
    assert(paths.has(required), `packed artifact is missing ${required}`);
  }
  for (const path of paths) {
    assert.match(path, /^(README\.md|LICENSE|NOTICE|package\.json|dist\/)/);
  }
  for (const path of paths) {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "pixelkit-laya-package-consumer", private: true, type: "module" }),
  );
  await runNpm(
    [
      "install",
  ...
  await writeFile(
    ...
    `import assert from "node:assert/strict";
import { Agent, VERSION } from "@pixelkit-labs/laya";
...
assert.equal(typeof loadMobileAgent, "function");
